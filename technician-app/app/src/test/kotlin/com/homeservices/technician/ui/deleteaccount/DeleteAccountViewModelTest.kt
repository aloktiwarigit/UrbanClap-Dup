...
public class DeleteAccountViewModelTest {
    ...
    @BeforeEach public fun setUp() { Dispatchers.setMain(dispatcher) }
    @AfterEach public fun tearDown() { Dispatchers.resetMain() }

    private fun vm() = DeleteAccountViewModel(submitUseCase)

    @Test public fun `initial state is Idle`(): Unit = runTest {
        val vm = vm()
        assertThat(vm.uiState.value).isEqualTo(DeleteAccountUiState.Idle)
    }

    @Test public fun `onConfirmDelete transitions through Submitting to Done on success`(): Unit = runTest {
    ...
    @Test public fun `onConfirmDelete sets ActiveJobBlocked when use case returns ActiveJobExists`(): Unit = runTest {
    ...
    @Test public fun `onConfirmDelete sets Error on DuplicatePending`(): Unit = runTest {
    ...
    @Test public fun `onConfirmDelete sets Error on UnknownError`(): Unit = runTest {
    ...
    @Test public fun `onDismissError resets to Idle`(): Unit = runTest {
