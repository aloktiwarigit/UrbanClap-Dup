...
public class LanguageSettingsViewModelTest {
    ...
    @AfterEach public fun tearDown(): Unit { Dispatchers.resetMain() }

    private fun vm(): LanguageSettingsViewModel = LanguageSettingsViewModel(getCurrentLocale, setAppLocale)

    @Test public fun `init loads current locale`(): Unit = runTest {
        val vm = vm(); advanceUntilIdle()
        assertEquals("en", vm.selectedTag.value)
    }

    @Test public fun `onSelect updates selectedTag`(): Unit = runTest {
        val vm = vm(); advanceUntilIdle()
        vm.onSelect("hi")
        assertEquals("hi", vm.selectedTag.value)
    }

    @Test public fun `onSave calls setAppLocale with selected tag`(): Unit = runTest {
        val vm = vm(); advanceUntilIdle()
        vm.onSelect("hi"); vm.onSave(); advanceUntilIdle()
        coVerify { setAppLocale("hi") }
    }

    @Test public fun `onSave sets savedFlow true`(): Unit = runTest {
        val vm = vm(); advanceUntilIdle()
