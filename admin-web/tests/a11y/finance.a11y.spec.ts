...
test('finance page has no critical/serious WCAG 2.1 AA violations', async ({ page, context, baseURL }) => {
  ...
  const results = await new AxeBuilder({ page })
    ...
    // th scope (P1-6) deferred to a later story
...
test('ApproveAllModal traps focus and closes on Escape', async ({ page, context, baseURL }) => {
  await signInAs(context, baseURL, 'super-admin');
  await page.route('**/admin-api/v1/admin/finance/payout-queue*', route =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
        entries: [{ technicianId: 'tech-1', technicianName: 'Suresh', netPayable: 10000, ordersCount: 1 }],
