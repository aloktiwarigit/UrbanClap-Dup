...
public class PlacesAutocompleteUseCaseTest {
    ...
    @Test
    public fun `findPredictions_returnsList_whenSdkSuccess`(): Unit =
        runTest {
            coEvery { gateway.findAutocompletePredictions(any(), any()) } returns
                listOf(
                    RawPrediction(
                        placeId = "place-1",
    ...
    @Test
    public fun `findPredictions_emptyOnSdkApiError_doesNotThrow`(): Unit =
        runTest {
    ...
    @Test
    public fun `findPredictions_propagatesTimeoutAsError`(): Unit =
        runTest {
    ...
    @Test
    public fun `fetchPlace_returnsPlaceWithLatLng_andRotatesSessionToken`(): Unit =
        runTest {
            val rawPlace =
                RawPlace(
                    placeId = "place-1",
            ...
            assertThat(useCase.currentToken).isNotSameAs(tokenBefore)
        }

    @Test
    public fun `sessionToken_isStableAcrossPredictionsCalls_untilFetchPlaceSucceeds`(): Unit =
        runTest {
            coEvery { gateway.findAutocompletePredictions(any(), any()) } returns emptyList()
            coEvery { gateway.fetchPlace(any(), any()) } returns
                RawPlace(
                    placeId = "p",
                    name = "Place",
                    address = "Addr",
