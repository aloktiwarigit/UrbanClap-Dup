...
**Current bundle on `main`:** `0.1.8` / versionCode 13
**Prepared:** 2026-08-14 — **§0 and §12 refreshed 2026-09-19**
...
> **Status 2026-09-19 — all four code blockers in this section are now RESOLVED on `main`.**
> Kept below for the reasoning and evidence, not as an open action list.
>
> | Blocker | Resolved by |
> |---|---|
> | 0.1 Target API level 36 | **#366** — `targetSdk 36`. Note `compileSdk` stays **35**: raising it to 36 breaks Paparazzi 1.3.5 outright (`NoSuchElementException` in `Renderer.kt`), and Play reads only `targetSdk`. See `docs/adr/0037-targetsdk-36-on-compilesdk-35.md`. |
> | 0.2 Play Billing warning | never applied — unchanged |
> | 0.3 Launcher name placeholder | **#370** — `app_name` = `HomeHeroo` in both locales |
> | 0.4 Privacy / deletion URLs | **#370** — `PRIVACY_POLICY_URL` repointed and re-verified HTTP 200 |
>
> A fifth Play rejection not listed in this section — the **16KB page-size** check — was
> resolved in **#369** by bumping Sentry 7.17.0 → 8.47.0. The old bundles carry
> `PT_LOAD` alignment `0x1000`; the new ones `0x4000`.
>
> The **PR #314 references below are stale** — that PR was closed unmerged on 2026-09-19
> because its versionCode bumps had fallen behind `main`. The fixes it contained were
> cherry-picked onto `main` instead, as listed above.

### 0.1 [RESOLVED 2026-09-19 — PR #366] Target API level 36 — hard deadline Aug 30, 2026
...
1. ~~**[code]** Bump `compileSdk`/`targetSdk` to 36 + edge-to-edge inset pass + re-record goldens on CI.~~ **DONE 2026-09-19** - PR #366. `targetSdk 36`, `compileSdk` deliberately left at 35 (ADR-0037); edge-to-edge audit across 10 screens; CatalogueHomeScreen goldens re-recorded on CI in PR #370.
1b. ~~**[code]** Clear the Play 16KB page-size rejection.~~ **DONE 2026-09-19** - PR #369, Sentry 7.17.0 -> 8.47.0. Verified by reading ELF `PT_LOAD` alignment: old bundle `0x1000`, new `0x4000`.
2. ~~**[code]** Fix `app_name` in both locales.~~ **DONE 2026-09-19** - landed on `main` via PR #370 (PR #314 itself was closed unmerged), set to `HomeHeroo` in `values/` and `values-hi/` (brand stays Latin, matching `HomeHeroo तकनीशियन`).
3. ~~**[code]** Point `PRIVACY_POLICY_URL` at the real GitHub Pages URL.~~ **DONE 2026-09-19** - landed on `main` via PR #370 (PR #314 itself was closed unmerged). Was `https://homeservices.app/privacy`, a domain that does not resolve.
...
10. **[build]** Rebuild both AABs on top of `main` (customer is already at versionCode 13, technician at 16), verify `targetSdkVersion 36` in the built artifact with `bundletool dump manifest` rather than trusting the source, then fill the Console using this document.
