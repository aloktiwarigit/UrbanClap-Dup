...
session id: 019dca30-82cc-7f23-9f29-609a154d2726
--------
user
...
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'git diff --stat 146834c689c0def763a11513397b7ce2efbb2693' in C:\Alok\Business Projects\Urbanclap-dup
 ...
 api/src/functions/trigger-no-show-detector.ts      | 149 ++++
 api/src/schemas/booking.ts                         |   3 +-
 api/src/schemas/customer-credit.ts                 |  12 +
 api/src/schemas/order.ts                           |   7 +-
 api/src/services/dispatcher.service.ts             | 125 +--
 ...
 api/tests/unit/dispatcher.service.test.ts          |  98 +++
 api/tests/unit/trigger-no-show-detector.test.ts    | 247 ++++++
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse api/src | Select-String -Pattern 'NO_SHOW_REDISPATCH|noShowRedispatchAt|redispatch\\('" in C:\Alok\Business Projects\Urbanclap-dup
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse api/src | Select-String -Pattern 'CANCELLED|CUSTOMER_CANCELLED|UNFULFILLED|PENDING_PAYMENT|AWAITING_PRICE_APPROVAL|CLOSED'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 315ms:
...
api\src\cosmos\booking-repository.ts:56:    if (!existing || (existing.status !== 'SEARCHING' && existing.status !== 
...
api\src\functions\bookings.ts:89:    console.error('[requestAddon] FCM push failed — booking is 
...
api\src\functions\rating-escalate.ts:34:  if (booking.status !== 'CLOSED') return { status: 409, jsonBody: { code: 
'BOOKING_NOT_CLOSED' } };
api\src\functions\ratings.ts:38:  if (booking.status !== 'CLOSED') {
api\src\functions\ratings.ts:39:    return { status: 409, jsonBody: { code: 'BOOKING_NOT_CLOSED', status: 
booking.status } };
api\src\functions\trigger-no-show-detector.ts:117:          ctx.log(`detectNoShows: no techs found for ${booking.id} — 
...
api\src\cosmos\booking-repository.ts:73:        query: "SELECT * FROM c WHERE c.status IN ('ASSIGNED', 
'NO_SHOW_REDISPATCH') AND c.slotDate <= @slotDate",
api\src\functions\trigger-no-show-detector.ts:11:const NO_SHOW_REDISPATCH_RADIUS_KM = 15;
api\src\functions\trigger-no-show-detector.ts:71:    // `noShowRedispatchAt` is stamped after a confirmed successful 
redispatch — if present,
api\src\functions\trigger-no-show-detector.ts:79:      if (liveBooking?.noShowRedispatchAt) {
api\src\functions\trigger-no-show-detector.ts:80:        ctx.log(`detectNoShows: recovery skipped for ${booking.id} — 
redispatch already fired at ${liveBooking.noShowRedispatchAt}`);
api\src\functions\trigger-no-show-detector.ts:88:        // A replacement tech accepted the redispatch; 
noShowRedispatchAt stamp failed but we're done
api\src\functions\trigger-no-show-detector.ts:95:    // noShowRedispatchAt, so recovery can retry both the status 
write and redispatch.
api\src\functions\trigger-no-show-detector.ts:98:      await updateBookingFields(booking.id, { status: 
'NO_SHOW_REDISPATCH' });
api\src\functions\trigger-no-show-detector.ts:106:    // against a booking that is still ASSIGNED 
(dispatcher.redispatch checks for NO_SHOW_REDISPATCH).
api\src\functions\trigger-no-show-detector.ts:107:    // Only stamp noShowRedispatchAt when redispatch actually sent 
offers (boolean true return)
api\src\functions\trigger-no-show-detector.ts:112:        redispatchOk = await 
dispatcherService.redispatch(booking.id, NO_SHOW_REDISPATCH_RADIUS_KM);
api\src\functions\trigger-no-show-detector.ts:115:          await updateBookingFields(booking.id, { 
noShowRedispatchAt: new Date().toISOString() });
api\src\schemas\booking.ts:7:  'UNFULFILLED', 'CUSTOMER_CANCELLED', 'NO_SHOW_REDISPATCH',
api\src\schemas\booking.ts:36:  noShowRedispatchAt: z.string().optional(),
api\src\schemas\order.ts:9:  'UNFULFILLED', 'CUSTOMER_CANCELLED', 'NO_SHOW_REDISPATCH',
api\src\services\dispatcher.service.ts:109:  async redispatch(bookingId: string, radiusKm: number): Promise<boolean> {
api\src\services\dispatcher.service.ts:111:    if (!booking || booking.status !== 'NO_SHOW_REDISPATCH') return false;



exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content docs/stories/E07-S04-no-show-detector.md | Select-Object -First 220' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 227ms:
# Story E07-S04: Tech no-show detection + auto-credit + re-dispatch
...
> **Sprint:** S4 (wk 7â€“8) Â· **Estimated:** â‰¤ 1 dev-day Â· **Priority:** P1
> **Sub-projects:** `api/` only â€” no Android or admin-web changes
> **Ceremony tier:** Feature (new Cosmos container + new Azure Function timer + dispatcher extension â€” substantive but bounded; Codex + CI; no /security-review)
> **Prerequisite:** E05-S02 (dispatcher live) + E06-S04 (wallet_ledger pattern established)
...
As a **customer whose technician never arrived for the booked slot**,
I want the platform to automatically detect the no-show after 30 minutes, credit â‚¹500 to my account, and find me a new technician â€” without any action on my part,
so that **I am compensated fairly and not stuck waiting for a tech who won't come**.
...
### AC-1 Â· No-show detection fires exactly once per booking
- **Given** a booking is in `ASSIGNED` status (tech accepted but hasn't transitioned to `REACHED`)
- **When** 30 minutes have elapsed since the slot start time (derived from `booking.slotDate` + start of `booking.slotWindow` in IST)
- **Then** the no-show detector processes that booking exactly once
- **And** subsequent timer invocations skip already-processed bookings (idempotency via 409 Conflict on `customer_credits` insert with `id = bookingId`)

### AC-2 Â· â‚¹500 credit written to `customer_credits` container
- **When** a no-show is detected for `bookingId`
- **Then** a document is created in the `customer_credits` Cosmos container:
  ```
  { id: bookingId, customerId, bookingId, amount: 50000, reason: "NO_SHOW", createdAt }
  ```
  (amount in paise: 50000 = â‚¹500)
- **And** if the document already exists (409), the booking is left unchanged â€” no double credit

### AC-3 Â· Booking status transitions to `NO_SHOW_REDISPATCH`
- **When** the credit is written successfully
- **Then** `booking.status` is updated from `ASSIGNED` â†’ `NO_SHOW_REDISPATCH`
- **And** this transition is idempotent â€” if the booking is already `NO_SHOW_REDISPATCH`, skip

### AC-4 Â· Re-dispatch with expanded radius
- **When** the booking transitions to `NO_SHOW_REDISPATCH`
- **Then** the dispatcher is called with `radiusKm = 15` (= original `DISPATCH_RADIUS_KM * 1.5`)
- **And** the dispatcher creates a new `DispatchAttemptDoc` and FCMs the top-3 techs in the expanded radius
- **And** the booking status transitions to `SEARCHING` (owned by the dispatcher, same as normal dispatch)

### AC-5 Â· FCM notification to customer
- **When** the credit is written and re-dispatch is triggered
- **Then** an FCM data message is sent to `customer_{customerId}` topic:
  ```json
  {
  ...
  ```
- FCM failure must be caught and captured by Sentry â€” it must NOT roll back the credit or block re-dispatch

### AC-6 Â· Timer schedule
- The function runs every 5 minutes: cron `0 */5 * * * *`
- Must handle the case where no bookings need processing (zero iterations â€” no error)
...
- [ ] **T1 â€” Schema + Cosmos client (api/, no tests)**
  - [ ] Add `'NO_SHOW_REDISPATCH'` to `BOOKING_STATUSES` array in `api/src/schemas/booking.ts`
  - [ ] Create `api/src/schemas/customer-credit.ts` â€” `CustomerCreditDocSchema` + `CustomerCreditDoc` type
  - [ ] Add `getCustomerCreditsContainer()` to `api/src/cosmos/client.ts`

- [ ] **T2 â€” `customerCreditRepo` (api/, TDD)**
  - [ ] Create `api/src/cosmos/customer-credit-repository.ts`
  - [ ] `createCreditIfAbsent(doc): Promise<boolean>` â€” returns `false` (already exists) on 409, throws on other errors
  - [ ] Test file: `api/tests/unit/cosmos/customer-credit-repository.test.ts`

- [ ] **T3 â€” Booking repo: ASSIGNED query (api/, TDD)**
  - [ ] Add `getAssignedBookingsBefore(slotDateCutoff: string): Promise<BookingDoc[]>` to `api/src/cosmos/booking-repository.ts`
  - [ ] Queries: `SELECT * FROM c WHERE c.status = 'ASSIGNED' AND c.slotDate <= @slotDate`
  - [ ] Test file: `api/tests/unit/cosmos/booking-repository.test.ts` (extend existing if present, or new file)

- [ ] **T4 â€” Dispatcher re-dispatch variant (api/, TDD)**
  - [ ] In `api/src/services/dispatcher.service.ts`, export `redispatch(bookingId: string, radiusKm: number): Promise<void>`
  - [ ] Reuses internal dispatch logic but accepts a radius override and skips the PAID status guard (caller has already transitioned booking to `NO_SHOW_REDISPATCH`)
  - [ ] Refactor: extract `dispatchBookingToTechs(bookingId, radiusKm, booking)` helper called by both `triggerDispatch` and `redispatch`
  - [ ] Test: `api/tests/unit/dispatcher.service.test.ts` â€” extend with `redispatch` cases

- [ ] **T5 â€” FCM: `sendNoShowCreditPush` (api/, no separate test)**
  - [ ] Add `sendNoShowCreditPush(customerId: string, bookingId: string, creditAmount: number): Promise<void>` to `api/src/services/fcm.service.ts`
  - [ ] Topic: `` `customer_${customerId}` ``, data: `{ type: 'NO_SHOW_CREDIT_ISSUED', bookingId, creditAmount: String(creditAmount), bodyText: '...' }`
  - [ ] Covered by integration in T6 test

- [ ] **T6 â€” `trigger-no-show-detector` Azure Function (api/, TDD)**
  - [ ] Create `api/src/functions/trigger-no-show-detector.ts`
  - [ ] Export `detectNoShows(ctx: InvocationContext): Promise<void>` (testable standalone)
  - [ ] Register `app.timer('triggerNoShowDetector', { schedule: '0 */5 * * * *', handler })`
  - [ ] Test file: `api/tests/unit/trigger-no-show-detector.test.ts` (see Dev Notes for test matrix)

- [ ] **T7 â€” Pre-Codex smoke gate + Codex review**
  - [ ] `bash tools/pre-codex-smoke-api.sh` â€” must exit 0
...
Parse slot start in IST (India does not observe DST â€” always +05:30):
```typescript
function slotStartUtcMs(slotDate: string, slotWindow: string): number {
  const startTime = slotWindow.split('-')[0]; // '10:00'
  // Append IST offset so JS Date parses correctly
...
**Query strategy:** Cosmos cannot compute this derivation server-side. Query all `ASSIGNED` bookings where `slotDate <= today` (using today's IST date, e.g. `new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' })`), then filter in code using the helper above. At pilot scale (â‰¤5k bookings/mo) this cross-partition scan is acceptable â€” revisit only if ASSIGNED backlog exceeds hundreds of docs.

### `NO_SHOW_REDISPATCH` status must be added to enum

`api/src/schemas/booking.ts:4` â€” `BOOKING_STATUSES` const array does NOT include `NO_SHOW_REDISPATCH`. Add it. Do NOT change the `confirmPayment` or `markPaid` methods â€” they have hardcoded status checks that are correct as-is.

### Dispatcher re-dispatch: bypass the PAID guard

`dispatcherService.triggerDispatch` (line 34â€“38) returns early if `booking.status !== 'PAID'`. For re-dispatch, the booking is already `NO_SHOW_REDISPATCH`. The cleanest approach:

```typescript
// Extract inner logic â€” status-agnostic
async function dispatchBookingToTechs(
  bookingId: string,
  booking: BookingDoc,
  radiusKm: number,
...
export const dispatcherService = {
  ...
  async redispatch(bookingId: string, radiusKm: number): Promise<void> {
    const booking = await bookingRepo.getById(bookingId);
    if (!booking) return; // booking deleted â€” shouldn't happen
    await dispatchBookingToTechs(bookingId, booking, radiusKm);
  },
};
```

The `DISPATCH_RADIUS_KM = 10` constant stays unchanged. The no-show detector passes `DISPATCH_RADIUS_KM * 1.5 = 15` explicitly. Do NOT import the constant from dispatcher in trigger-no-show-detector.ts â€” hardcode `15` or define `NO_SHOW_REDISPATCH_RADIUS_KM = 15` locally in the trigger file to avoid tight coupling.
...
export const CustomerCreditDocSchema = z.object({
  ...
  customerId: z.string(),   // partition key
  bookingId: z.string(),
  amount: z.number().int().positive(), // paise: 50000 = â‚¹500
  reason: z.literal('NO_SHOW'),
  createdAt: z.string(),
});
```

Partition key: `/customerId` â€” allows future "get all credits for customer" queries.
Container accessor: `getCustomerCreditsContainer()` in `api/src/cosmos/client.ts` â€” identical pattern to `getWalletLedgerContainer()` (line 43â€“45).

### Idempotency: 409 Conflict on duplicate insert

Reuse the `wallet-ledger-repository.ts` pattern (line 28â€“31):
```typescript
async createCreditIfAbsent(doc: CustomerCreditDoc): Promise<boolean> {
  try {
    await getCustomerCreditsContainer().items.create(doc);
...
```
If `createCreditIfAbsent` returns `false`, the no-show was already processed â€” skip the rest. This is the sole idempotency gate. Do NOT use `_etag` here â€” 409 on `id` uniqueness is simpler and sufficient.

### No-show detector: execution order matters

For each booking in the ASSIGNED-before-cutoff set:
1. `createCreditIfAbsent` â†’ if false, skip (already processed)
2. `updateBookingFields(bookingId, { status: 'NO_SHOW_REDISPATCH' })` â€” best-effort; if it fails, Sentry capture + continue
3. `dispatcherService.redispatch(bookingId, 15)` â€” best-effort; if it fails, Sentry capture + continue
4. `sendNoShowCreditPush(customerId, bookingId, 50000)` â€” best-effort; if it fails, Sentry capture + continue (FCM failure must NOT undo credit)

The credit write (step 1) is the atomic gate. Steps 2â€“4 are best-effort. This matches the `reconcilePayouts` pattern in `trigger-reconcile-payouts.ts`.
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content docs/stories/E07-S04-no-show-detector.md | Select-Object -Skip 220 -First 120' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 225ms:
      bookingId,
      creditAmount: String(creditAmount),
      bodyText: 'à¤¤à¤•à¤¨à¥€à¤¶à¤¿à¤¯à¤¨ à¤¨à¤¹à¥€à¤‚ à¤†à¤ â€” â‚¹500 credit à¤†à¤ªà¤•à¥‡ account à¤®à¥‡à¤‚ à¤œà¥‹à¤¡à¤¼à¤¾ à¤—à¤¯à¤¾à¥¤ à¤¨à¤¯à¤¾ à¤¤à¤•à¤¨à¥€à¤¶à¤¿à¤¯à¤¨ à¤¢à¥‚à¤‚à¤¢ à¤°à¤¹à¥‡ à¤¹à¥ˆà¤‚à¥¤',
...
The `bodyText` field is for client display â€” Android app parses it from the data payload. No notification object needed (the app handles foreground rendering).
...
|---|---|
| No ASSIGNED bookings | No credit, no FCM, no dispatch |
| ASSIGNED booking, slot 15 min ago | Not yet due â€” skip |
| ASSIGNED booking, slot 45 min ago | Credit written, status â†’ NO_SHOW_REDISPATCH, redispatch(id, 15), FCM sent |
| `createCreditIfAbsent` returns false (409) | Skip all subsequent steps for that booking |
| `updateBookingFields` throws | Sentry.captureException, continue to next booking |
| `redispatch` throws | Sentry.captureException, FCM still sent |
| `sendNoShowCreditPush` throws | Sentry.captureException, no rethrow |
...
`'10:00-12:00'.split('-')` â†’ `['10:00', '12:00']` âœ“  
The split character `-` does not appear inside the time components (`HH:MM`), only between them. This is safe.
...
- [ ] `cd api && pnpm typecheck && pnpm lint && pnpm test:coverage` green (â‰¥80%)
- [ ] All 8 scenarios in T6 test matrix covered by assertions
- [ ] AC-1 through AC-6 verified via test assertions
- [ ] Pre-Codex smoke gate exits 0
- [ ] `.codex-review-passed` marker present
...
**UPDATED files:**
- `api/src/schemas/booking.ts` â€” add `'NO_SHOW_REDISPATCH'` to `BOOKING_STATUSES`
- `api/src/cosmos/client.ts` â€” add `getCustomerCreditsContainer()`
- `api/src/cosmos/booking-repository.ts` â€” add `getAssignedBookingsBefore()`
- `api/src/services/dispatcher.service.ts` â€” extract `dispatchBookingToTechs()`, add `redispatch()`
- `api/src/services/fcm.service.ts` â€” add `sendNoShowCreditPush()`

exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse api/src | Select-String -Pattern 'getLatestByBookingId\\('" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 221ms:
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content api/src/cosmos/dispatch-attempt-repository.ts' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 216ms:
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse api/src | Select-String -Pattern 'dispatchAttemptRepo'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 258ms:
...
api\src\functions\job-offers.ts:25:  const attempt = await dispatchAttemptRepo.getByBookingId(bookingId);
api\src\functions\job-offers.ts:36:  const accepted = await dispatchAttemptRepo.acceptAttempt(attempt.id, bookingId);
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content api/src/cosmos/booking-repository.ts | Select-Object -First 140' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 236ms:
import { randomUUID } from 'node:crypto';
...
2026-04-26T14:29:24.233748Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'NO_SHOW_REDISPATCH|OrderStatus|BookingStatus|ActiveJobStatus|JobStatus'"` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'NO_SHOW_REDISPATCH|OrderStatus|BookingStatus|ActiveJobStatus|JobStatus'" in C:\Alok\Business Projects\Urbanclap-dup
 declined in 0ms:
`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'NO_SHOW_REDISPATCH|OrderStatus|BookingStatus|ActiveJobStatus|JobStatus'"` rejected: blocked by policy
2026-04-26T14:29:24.346296Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'UNFULFILLED|CUSTOMER_CANCELLED|AWAITING_PRICE_APPROVAL|CLOSED|PAID|ASSIGNED|SEARCHING'"` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'UNFULFILLED|CUSTOMER_CANCELLED|AWAITING_PRICE_APPROVAL|CLOSED|PAID|ASSIGNED|SEARCHING'" in C:\Alok\Business Projects\Urbanclap-dup
 declined in 0ms:
`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse customer-app,technician-app -Include *.kt | Select-String -Pattern 'UNFULFILLED|CUSTOMER_CANCELLED|AWAITING_PRICE_APPROVAL|CLOSED|PAID|ASSIGNED|SEARCHING'"` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Path customer-app -Recurse -Filter *.kt | Select-String -Pattern 'NO_SHOW_REDISPATCH|OrderStatus|BookingStatus|ActiveJobStatus|JobStatus'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 1172ms:

customer-app\app\src\main\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImpl.kt:4:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\main\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImpl.kt:22:                
.scan(TrackingState(location = null, status = BookingStatus.EnRoute)) { state, event ->
customer-app\app\src\main\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImpl.kt:36:                 
           state.copy(status = BookingStatus.fromFcmString(event.status))
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCase.kt:3:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCase.kt:8:public class 
TrackBookingStatusUseCase
...
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:3:public sealed 
class BookingStatus {
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:4:    public object 
EnRoute : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:6:    public object 
Reached : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:8:    public object 
InProgress : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:10:    public object 
Completed : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:12:    public object 
Cancelled : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:14:    public object 
Unknown : BookingStatus()
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\BookingStatus.kt:17:        public 
fun fromFcmString(value: String): BookingStatus =
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\tracking\model\TrackingState.kt:5:    val status: 
BookingStatus,
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:38:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:156:private fun 
StatusTimeline(currentStatus: BookingStatus) {
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:159:            
BookingStatus.EnRoute to "En route",
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:160:            
BookingStatus.Reached to "Technician arrived",
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:161:            
BookingStatus.InProgress to "Work in progress",
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingScreen.kt:162:            
BookingStatus.Completed to "Completed",
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingUiState.kt:3:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingUiState.kt:11:        val status: 
BookingStatus,
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModel.kt:7:import 
com.homeservices.customer.domain.tracking.TrackBookingStatusUseCase
customer-app\app\src\main\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModel.kt:21:        private val 
trackBookingStatusUseCase: TrackBookingStatusUseCase,
...
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:3:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:16:    public 
fun `BookingStatus fromFcmString maps EN_ROUTE`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:17:        
assertThat(BookingStatus.fromFcmString("EN_ROUTE")).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:21:    public 
fun `BookingStatus fromFcmString maps REACHED`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:22:        
assertThat(BookingStatus.fromFcmString("REACHED")).isEqualTo(BookingStatus.Reached)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:26:    public 
fun `BookingStatus fromFcmString maps IN_PROGRESS`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:27:        
assertThat(BookingStatus.fromFcmString("IN_PROGRESS")).isEqualTo(BookingStatus.InProgress)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:31:    public 
fun `BookingStatus fromFcmString maps COMPLETED`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:32:        
assertThat(BookingStatus.fromFcmString("COMPLETED")).isEqualTo(BookingStatus.Completed)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:36:    public 
fun `BookingStatus fromFcmString maps CANCELLED`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:37:        
assertThat(BookingStatus.fromFcmString("CANCELLED")).isEqualTo(BookingStatus.Cancelled)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:41:    public 
fun `BookingStatus fromFcmString returns Unknown for unrecognised string`() {
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:42:        
assertThat(BookingStatus.fromFcmString("GARBAGE")).isEqualTo(BookingStatus.Unknown)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:71:            
assertThat(results[0].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:75:            
assertThat(results[1].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:90:            
assertThat(results[0].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:92:            
assertThat(results[1].status).isEqualTo(BookingStatus.Reached)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:108:            
assertThat(results[0].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:110:            
assertThat(results[1].status).isEqualTo(BookingStatus.InProgress)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:126:            
assertThat(results[0].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:129:            
assertThat(results[1].status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\data\tracking\TrackingRepositoryImplTest.kt:132:            
assertThat(results[2].status).isEqualTo(BookingStatus.Reached)
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\GetLiveLocationUseCaseTest.kt:3:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\GetLiveLocationUseCaseTest.kt:23:           
         TrackingState(location = null, status = BookingStatus.EnRoute),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\GetLiveLocationUseCaseTest.kt:35:           
         TrackingState(location = loc, status = BookingStatus.InProgress),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\GetLiveLocationUseCaseTest.kt:47:           
         TrackingState(null, BookingStatus.EnRoute),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\GetLiveLocationUseCaseTest.kt:48:           
         TrackingState(loc, BookingStatus.EnRoute),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:3:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:13:public 
class TrackBookingStatusUseCaseTest {
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:15:    
private val useCase = TrackBookingStatusUseCase(repo)
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:22:        
            TrackingState(location = null, status = BookingStatus.EnRoute),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:25:        
    assertThat(result).containsExactly(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:33:        
            TrackingState(null, BookingStatus.EnRoute),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:34:        
            TrackingState(null, BookingStatus.Reached),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:35:        
            TrackingState(null, BookingStatus.InProgress),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:36:        
            TrackingState(null, BookingStatus.Completed),
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:40:        
        BookingStatus.EnRoute,
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:41:        
        BookingStatus.Reached,
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:42:        
        BookingStatus.InProgress,
customer-app\app\src\test\kotlin\com\homeservices\customer\domain\tracking\TrackBookingStatusUseCaseTest.kt:43:        
        BookingStatus.Completed,
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:5:import 
com.homeservices.customer.domain.tracking.TrackBookingStatusUseCase
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:6:import 
com.homeservices.customer.domain.tracking.model.BookingStatus
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:27:    private val 
trackStatus: TrackBookingStatusUseCase = mockk()
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:48:            
every { trackStatus.execute(any()) } returns flowOf(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:58:            
every { trackStatus.execute("b1") } returns flowOf(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:65:            
assertThat(tracking.status).isEqualTo(BookingStatus.EnRoute)
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:73:            
every { trackStatus.execute("b2") } returns flowOf(BookingStatus.InProgress)
customer-app\app\src\test\kotlin\com\homeservices\customer\ui\tracking\LiveTrackingViewModelTest.kt:78:            
assertThat(tracking.status).isEqualTo(BookingStatus.InProgress)



exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Recurse api/src | Select-String -Pattern 'status: .*booking.status|BOOKING_STATUS|booking status|track|customer_'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 472ms:
...
api\src\cosmos\rating-repository.ts:34:        ...(input.side === 'CUSTOMER_TO_TECH'
api\src\cosmos\rating-repository.ts:53:      input.side === 'CUSTOMER_TO_TECH'
api\src\cosmos\rating-repository.ts:60:      ...(input.side === 'CUSTOMER_TO_TECH'
api\src\functions\active-job-photos.ts:13:// Each booking status maps to exactly one valid photo stage: the next 
transition target.
api\src\functions\active-job.ts:46:      status: booking.status,
api\src\functions\bookings.ts:63:      bookingId: booking.id, status: booking.status, amount: booking.amount,
api\src\functions\ratings.ts:36:  if (data.side === 'CUSTOMER_TO_TECH' && !isCustomer) return { status: 403, jsonBody: 
{ code: 'FORBIDDEN' } };
api\src\functions\ratings.ts:39:    return { status: 409, jsonBody: { code: 'BOOKING_NOT_CLOSED', status: 
booking.status } };
api\src\functions\trigger-no-show-detector.ts:94:    // Step 1: Set the holding status. Track success — if this fails 
we must NOT stamp
api\src\schemas\booking.ts:4:const BOOKING_STATUSES = [
api\src\schemas\booking.ts:7:  'UNFULFILLED', 'CUSTOMER_CANCELLED', 'NO_SHOW_REDISPATCH',
api\src\schemas\booking.ts:21:  status: z.enum(BOOKING_STATUSES),
api\src\schemas\order.ts:9:  'UNFULFILLED', 'CUSTOMER_CANCELLED', 'NO_SHOW_REDISPATCH',
api\src\schemas\rating.ts:17:    side: z.literal('CUSTOMER_TO_TECH'),
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Path technician-app -Recurse -Filter *.kt | Select-String -Pattern 'ActiveJobStatus|ASSIGNED|EN_ROUTE|REACHED|IN_PROGRESS|NO_SHOW_REDISPATCH|UNFULFILLED|SEARCHING'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 900ms:

technician-app\app\src\main\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImpl.kt:8:import 
...
technician-app\app\src\main\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImpl.kt:113:          
      status = ActiveJobStatus.valueOf(status),
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\ActiveJobRepository.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\ActiveJobRepository.kt:14:        
targetStatus: ActiveJobStatus,
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCase.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCase.kt:15:            
repository.transitionStatus(bookingId, ActiveJobStatus.COMPLETED)
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCase.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCase.kt:9:public class 
MarkReachedUseCase
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCase.kt:15:            
repository.transitionStatus(bookingId, ActiveJobStatus.REACHED)
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCase.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCase.kt:16:            val 
result = repository.transitionStatus(bookingId, ActiveJobStatus.EN_ROUTE)
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCase.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCase.kt:15:            
repository.transitionStatus(bookingId, ActiveJobStatus.IN_PROGRESS)
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJob.kt:10:    val status: 
ActiveJobStatus,
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJobStatus.kt:3:public enum 
class ActiveJobStatus {
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJobStatus.kt:4:    
ASSIGNED,
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJobStatus.kt:5:    
EN_ROUTE,
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJobStatus.kt:6:    REACHED,
technician-app\app\src\main\kotlin\com\homeservices\technician\domain\activeJob\model\ActiveJobStatus.kt:7:    
IN_PROGRESS,
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:27:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:152:                
ActiveJobAction.START_TRIP -> Triple("Start Trip", true, "EN_ROUTE")
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:153:                
ActiveJobAction.MARK_ARRIVED -> Triple("I've Arrived", true, "REACHED")
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:154:                
ActiveJobAction.START_WORK -> Triple("Start Work", true, "IN_PROGRESS")
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:173:    currentStatus: 
ActiveJobStatus,
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:178:            
ActiveJobStatus.ASSIGNED to "Assigned",
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:179:            
ActiveJobStatus.EN_ROUTE to "En Route",
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:180:            
ActiveJobStatus.REACHED to "Arrived",
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:181:            
ActiveJobStatus.IN_PROGRESS to "Working",
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreen.kt:182:            
ActiveJobStatus.COMPLETED to "Done",
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:9:import 
com.homeservices.technician.domain.activeJob.MarkReachedUseCase
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:12:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:32:        private 
val markReachedUseCase: MarkReachedUseCase,
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:51:                  
      if (job.status == ActiveJobStatus.COMPLETED) {
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:92:        public 
fun markReached(): Unit {
...
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:178:                 
       "EN_ROUTE" -> {
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:183:                 
       "REACHED" -> markReachedUseCase(bookingId)
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:184:                 
       "IN_PROGRESS" -> startWorkUseCase(bookingId)
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:209:        private 
fun ActiveJobStatus.toAction(): ActiveJobAction =
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:211:                
ActiveJobStatus.ASSIGNED -> ActiveJobAction.START_TRIP
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:212:                
ActiveJobStatus.EN_ROUTE -> ActiveJobAction.MARK_ARRIVED
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:213:                
ActiveJobStatus.REACHED -> ActiveJobAction.START_WORK
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:214:                
ActiveJobStatus.IN_PROGRESS -> ActiveJobAction.COMPLETE_JOB
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModel.kt:215:                
ActiveJobStatus.COMPLETED -> ActiveJobAction.NONE
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\PhotoCaptureScreen.kt:257:        
"EN_ROUTE" -> "Starting Trip — Take a photo of your transport"
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\PhotoCaptureScreen.kt:258:        
"REACHED" -> "Arrived — Take a photo of the site"
technician-app\app\src\main\kotlin\com\homeservices\technician\ui\activeJob\PhotoCaptureScreen.kt:259:        
"IN_PROGRESS" -> "Starting Work — Take a photo of the work area"
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:9:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:30:    
private fun aResponse(status: String = "ASSIGNED") =
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:62:       
     coEvery { api.transitionStatus(any(), any(), any()) } returns Response.success(aResponse("EN_ROUTE"))
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:64:       
     val result = repo.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:75:       
     val result = repo.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:78:       
     coVerify(exactly = 1) { dao.insert(match { it.bookingId == "bk-1" && it.targetStatus == "EN_ROUTE" }) }
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:86:       
             PendingTransitionEntity("id-1", "bk-1", "EN_ROUTE", createdAt = 1000L),
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:87:       
             PendingTransitionEntity("id-2", "bk-1", "REACHED", createdAt = 2000L),
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:90:       
     coEvery { api.transitionStatus(any(), any(), any()) } returns Response.success(aResponse("EN_ROUTE"))
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:102:      
      val entry = PendingTransitionEntity("id-1", "bk-1", "IN_PROGRESS", createdAt = 1000L)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:128:      
              listOf(PendingTransitionEntity("id-1", "bk-1", "EN_ROUTE", 1000L)),
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:143:      
      val result = repo.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:154:      
      val result = repo.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:172:      
      val entry = PendingTransitionEntity("id-1", "bk-1", "EN_ROUTE", createdAt = 1000L)
technician-app\app\src\test\kotlin\com\homeservices\technician\data\activeJob\ActiveJobRepositoryImplTest.kt:184:      
      val entry = PendingTransitionEntity("id-1", "bk-1", "EN_ROUTE", createdAt = 1000L)
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCaseTest.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCaseTest.kt:25:          
  status = ActiveJobStatus.COMPLETED,
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCaseTest.kt:33:          
  coEvery { repository.transitionStatus("bk-1", ActiveJobStatus.COMPLETED) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\CompleteJobUseCaseTest.kt:39:          
  coVerify(exactly = 1) { repository.transitionStatus("bk-1", ActiveJobStatus.COMPLETED) }
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:12:public 
class MarkReachedUseCaseTest {
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:14:    
private val useCase = MarkReachedUseCase(repository)
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:24:          
  status = ActiveJobStatus.REACHED,
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:30:    
public fun `transitions EN_ROUTE to REACHED`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:32:          
  coEvery { repository.transitionStatus("bk-1", ActiveJobStatus.REACHED) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\MarkReachedUseCaseTest.kt:38:          
  assertThat(result.getOrThrow().status).isEqualTo(ActiveJobStatus.REACHED)
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCaseTest.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCaseTest.kt:17:    private 
fun aJob(status: ActiveJobStatus = ActiveJobStatus.EN_ROUTE) =
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCaseTest.kt:33:            
coEvery { repository.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCaseTest.kt:34:            
    Result.success(aJob(ActiveJobStatus.EN_ROUTE))
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartTripUseCaseTest.kt:45:            
coEvery { repository.transitionStatus("bk-1", ActiveJobStatus.EN_ROUTE) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCaseTest.kt:4:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCaseTest.kt:24:            
status = ActiveJobStatus.IN_PROGRESS,
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCaseTest.kt:30:    public 
fun `transitions REACHED to IN_PROGRESS`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCaseTest.kt:32:            
coEvery { repository.transitionStatus("bk-1", ActiveJobStatus.IN_PROGRESS) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\activeJob\StartWorkUseCaseTest.kt:38:            
assertThat(result.getOrThrow().status).isEqualTo(ActiveJobStatus.IN_PROGRESS)
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:14:    
private val storagePath = "bookings/b1/photos/uid123/REACHED/1234567890.jpg"
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:19:           
 coEvery { repository.uploadPhoto("b1", "REACHED", "/cache/photo.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:21:           
 coEvery { repository.recordPhotoPath("b1", "REACHED", storagePath) } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:24:           
 val result = useCase.execute("b1", "REACHED", "/cache/photo.jpg")
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:28:           
 coVerify(exactly = 1) { repository.recordPhotoPath("b1", "REACHED", storagePath) }
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:37:           
 val result = useCase.execute("b1", "REACHED", "/cache/photo.jpg")
technician-app\app\src\test\kotlin\com\homeservices\technician\domain\photo\UploadJobPhotoUseCaseTest.kt:52:           
 val result = useCase.execute("b1", "REACHED", "/cache/photo.jpg")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:5:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:14:    
private fun aJob(status: ActiveJobStatus) =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:31:        
        uiState = ActiveJobUiState.Active(aJob(ActiveJobStatus.EN_ROUTE), ActiveJobAction.MARK_ARRIVED),
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:41:    
public fun activeJobScreen_reached(): Unit {
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:44:        
        uiState = ActiveJobUiState.Active(aJob(ActiveJobStatus.REACHED), ActiveJobAction.START_WORK),
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobScreenPaparazziTest.kt:57:        
        uiState = ActiveJobUiState.Active(aJob(ActiveJobStatus.IN_PROGRESS), ActiveJobAction.COMPLETE_JOB),
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:7:import 
com.homeservices.technician.domain.activeJob.MarkReachedUseCase
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:11:import 
com.homeservices.technician.domain.activeJob.model.ActiveJobStatus
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:40:    private 
lateinit var markReachedUseCase: MarkReachedUseCase
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:47:    private 
fun aJob(status: ActiveJobStatus = ActiveJobStatus.ASSIGNED) =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:65:        
markReachedUseCase = mockk(relaxed = true)
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:79:              
  markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:106:             
       Result.success(aJob(ActiveJobStatus.EN_ROUTE)),
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:148:             
       markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:163:    public 
fun `markReached delegates to markReachedUseCase`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:165:            
viewModel.markReached()
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:166:            
coVerify(exactly = 1) { markReachedUseCase("bk-1") }
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:194:             
       markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:206:            
every { repository.getActiveJob("bk-1") } returns flowOf(aJob(ActiveJobStatus.COMPLETED))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:213:             
       markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:225:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "IN_PROGRESS", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:226:             
   Result.success("bookings/bk-1/photos/uid/IN_PROGRESS/123.jpg")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:229:            
viewModel.onTransitionRequested("IN_PROGRESS")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:235:            
assertThat(state.pendingPhotoStage).isEqualTo("IN_PROGRESS")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:243:            
val jobFlow = MutableStateFlow(aJob(ActiveJobStatus.REACHED))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:251:             
       markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:258:            
vm.onTransitionRequested("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:260:            
jobFlow.value = aJob(ActiveJobStatus.REACHED)
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:263:            
assertThat(state.pendingPhotoStage).isEqualTo("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:270:            
coEvery { startWorkUseCase("bk-1") } returns Result.success(aJob(ActiveJobStatus.IN_PROGRESS))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:273:            
viewModel.onTransitionRequested("IN_PROGRESS")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:283:             
       ?.copy(uploadedStoragePath = "bookings/bk-1/photos/uid/IN_PROGRESS/123.jpg") ?: return@runTest
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:286:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "IN_PROGRESS", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:287:             
   Result.success("bookings/bk-1/photos/uid/IN_PROGRESS/123.jpg")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:294:            
coEvery { startWorkUseCase("bk-1") } returns Result.success(aJob(ActiveJobStatus.IN_PROGRESS))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:308:            
viewModel.onTransitionRequested("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:318:            
assertThat(stateAfterRetake.pendingPhotoStage).isEqualTo("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:331:             
       markReachedUseCase,
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:353:            
viewModel.onTransitionRequested("EN_ROUTE")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:355:            
assertThat(state.pendingPhotoStage).isEqualTo("EN_ROUTE")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:362:            
viewModel.onTransitionRequested("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:372:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "IN_PROGRESS", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:374:            
coEvery { startWorkUseCase("bk-1") } returns Result.success(aJob(ActiveJobStatus.IN_PROGRESS))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:376:            
viewModel.onTransitionRequested("IN_PROGRESS")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:393:            
viewModel.onTransitionRequested("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:400:            
assertThat(state.pendingPhotoStage).isEqualTo("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:404:    public 
fun `onPhotoConfirmed EN_ROUTE fires startTrip and emits navigation event`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:406:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "EN_ROUTE", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:410:             
       Result.success(aJob(ActiveJobStatus.EN_ROUTE)),
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:417:            
viewModel.onTransitionRequested("EN_ROUTE")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:428:    public 
fun `onPhotoConfirmed REACHED fires markReachedUseCase`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:430:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "REACHED", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:432:            
coEvery { markReachedUseCase("bk-1") } returns Result.success(aJob(ActiveJobStatus.REACHED))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:434:            
viewModel.onTransitionRequested("REACHED")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:438:            
coVerify(exactly = 1) { markReachedUseCase("bk-1") }
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:446:            
coEvery { completeJobUseCase("bk-1") } returns Result.success(aJob(ActiveJobStatus.COMPLETED))
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:456:    public 
fun `onPhotoConfirmed EN_ROUTE with null navEvent — no navigation emitted`(): Unit =
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:458:            
coEvery { uploadJobPhotoUseCase.execute("bk-1", "EN_ROUTE", "/cache/p.jpg") } returns
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:460:            
coEvery { startTripUseCase("bk-1") } returns Pair(Result.success(aJob(ActiveJobStatus.EN_ROUTE)), null)
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\ActiveJobViewModelTest.kt:465:            
viewModel.onTransitionRequested("EN_ROUTE")
technician-app\app\src\test\kotlin\com\homeservices\technician\ui\activeJob\PhotoCaptureScreenTest.kt:19:              
  stage = "REACHED",



exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content api/src/functions/active-job.ts' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 174ms:
import { z } from 'zod';
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command 'Get-Content technician-app/app/src/main/kotlin/com/homeservices/technician/data/activeJob/ActiveJobRepositoryImpl.kt | Select-Object -First 170' in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 261ms:
...
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "Get-ChildItem -Path customer-app -Recurse -Filter *.kt | Select-String -Pattern 'valueOf\\(status\\)|enum class .*Status|status: String'" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 729ms:

customer-app\app\build\generated\ksp\debug\kotlin\com\homeservices\customer\data\booking\remote\dto\ApproveFinalPriceRe
sponseDtoJsonAdapter.kt:38:    var status: String? = null
customer-app\app\build\generated\ksp\debug\kotlin\com\homeservices\customer\data\booking\remote\dto\ConfirmBookingRespo
nseDtoJsonAdapter.kt:33:    var status: String? = null
customer-app\app\build\generated\ksp\debug\kotlin\com\homeservices\customer\data\booking\remote\dto\GetBookingResponseD
toJsonAdapter.kt:46:    var status: String? = null
customer-app\app\build\generated\ksp\debug\kotlin\com\homeservices\customer\data\rating\remote\dto\GetRatingResponseDto
JsonAdapter.kt:46:    var status: String? = null
customer-app\app\build\generated\ksp\debug\kotlin\com\homeservices\customer\data\rating\remote\dto\SidePayloadDtoJsonAd
apter.kt:51:    var status: String? = null
customer-app\app\build\generated\ksp\release\kotlin\com\homeservices\customer\data\booking\remote\dto\ApproveFinalPrice
ResponseDtoJsonAdapter.kt:38:    var status: String? = null
customer-app\app\build\generated\ksp\release\kotlin\com\homeservices\customer\data\booking\remote\dto\ConfirmBookingRes
ponseDtoJsonAdapter.kt:33:    var status: String? = null
customer-app\app\build\generated\ksp\release\kotlin\com\homeservices\customer\data\booking\remote\dto\GetBookingRespons
eDtoJsonAdapter.kt:46:    var status: String? = null
customer-app\app\build\generated\ksp\release\kotlin\com\homeservices\customer\data\rating\remote\dto\GetRatingResponseD
toJsonAdapter.kt:46:    var status: String? = null
customer-app\app\build\generated\ksp\release\kotlin\com\homeservices\customer\data\rating\remote\dto\SidePayloadDtoJson
Adapter.kt:51:    var status: String? = null
customer-app\app\src\main\kotlin\com\homeservices\customer\data\booking\remote\dto\BookingDtos.kt:42:    val status: 
String,
customer-app\app\src\main\kotlin\com\homeservices\customer\data\booking\remote\dto\BookingDtos.kt:57:    val status: 
String,
customer-app\app\src\main\kotlin\com\homeservices\customer\data\booking\remote\dto\BookingDtos.kt:77:    val status: 
String,
customer-app\app\src\main\kotlin\com\homeservices\customer\data\rating\remote\dto\RatingDtos.kt:22:    val status: 
String,
customer-app\app\src\main\kotlin\com\homeservices\customer\data\rating\remote\dto\RatingDtos.kt:32:    val status: 
String,
customer-app\app\src\main\kotlin\com\homeservices\customer\data\rating\remote\dto\RatingDtos.kt:40:            status 
= RatingSnapshot.Status.valueOf(status),
customer-app\app\src\main\kotlin\com\homeservices\customer\data\tracking\TrackingEvent.kt:17:        internal val 
status: String,
customer-app\app\src\main\kotlin\com\homeservices\customer\domain\rating\model\Rating.kt:43:    public enum class 
Status { PENDING, PARTIALLY_SUBMITTED, REVEALED }



2026-04-26T14:30:02.805663Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -First 140'` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -First 140' in C:\Alok\Business Projects\Urbanclap-dup
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -Skip 100 -First 40' in C:\Alok\Business Projects\Urbanclap-dup
 declined in 0ms:
`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -First 140'` rejected: blocked by policy
 declined in 0ms:
`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -Skip 100 -First 40'` rejected: blocked by policy
2026-04-26T14:30:02.806043Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command '$i=0; Get-Content api/src/functions/trigger-no-show-detector.ts | ForEach-Object { $i++; '"'{0,4}: {1}' -f "'$i, $_ } | Select-Object -Skip 100 -First 40'` rejected: blocked by policy
2026-04-26T14:30:07.188979Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "@'
...
for i,l in enumerate(p.read_text().splitlines(),1):
    if 90 <= i <= 130:
        print(f'{i}: {l}')
'@ | python -"` rejected: blocked by policy
2026-04-26T14:30:11.480317Z ERROR codex_core::tools::router: error=`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "python -c \"from pathlib import Path; p=Path('api/src/functions/trigger-no-show-detector.ts'); lines=p.read_text().splitlines(); [print(f'{i+1}: {lines[i]}') for i in range(90-1, min(130, len(lines))) ]\""` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "python -c \"from pathlib import Path; p=Path('api/src/functions/trigger-no-show-detector.ts'); lines=p.read_text().splitlines(); [print(f'{i+1}: {lines[i]}') for i in range(90-1, min(130, len(lines))) ]\"" in C:\Alok\Business Projects\Urbanclap-dup
 declined in 0ms:
`"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "python -c \"from pathlib import Path; p=Path('api/src/functions/trigger-no-show-detector.ts'); lines=p.read_text().splitlines(); [print(f'{i+1}: {lines[i]}') for i in range(90-1, min(130, len(lines))) ]\""` rejected: blocked by policy
exec
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "findstr /n \".*\" api\\src\\functions\\trigger-no-show-detector.ts" in C:\Alok\Business Projects\Urbanclap-dup
 succeeded in 198ms:
1:import '../bootstrap.js';
2:import { app } from '@azure/functions';
3:import type { Timer, InvocationContext } from '@azure/functions';
4:import * as Sentry from '@sentry/node';
5:import { bookingRepo, updateBookingFields } from '../cosmos/booking-repository.js';
6:import { customerCreditRepo } from '../cosmos/customer-credit-repository.js';
7:import { dispatcherService } from '../services/dispatcher.service.js';
8:import { sendNoShowCreditPush } from '../services/fcm.service.js';
9:
10:const NO_SHOW_CREDIT_PAISE = 50_000;
11:const NO_SHOW_REDISPATCH_RADIUS_KM = 15;
12:const NO_SHOW_WINDOW_MS = 30 * 60 * 1_000;
13:
14:function slotStartUtcMs(slotDate: string, slotWindow: string): number {
15:  const startTime = slotWindow.split('-')[0]; // '10:00' from '10:00-12:00'
16:  const ms = new Date(`${slotDate}T${startTime}:00+05:30`).getTime();
17:  if (isNaN(ms)) throw new Error(`invalid slotWindow "${slotWindow}" on slotDate "${slotDate}"`);
18:  return ms;
19:}
20:
21:export async function detectNoShows(ctx: InvocationContext): Promise<void> {
22:  // 'en-CA' locale gives YYYY-MM-DD format — identical to slotDate schema
23:  const todayIST = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
24:  const now = Date.now();
25:
26:  // Note: bookings with a slotDate > todayIST but a UTC slot start already past 30 min
27:  // (i.e., slotWindow starts before 05:30 IST) are excluded by this query. For the
28:  // Ayodhya pilot (service hours 08:00–20:00 IST), this edge case does not apply.
29:  const assignedBookings = await bookingRepo.getAssignedBookingsBefore(todayIST);
30:  ctx.log(`detectNoShows: ${assignedBookings.length} ASSIGNED bookings on/before ${todayIST}`);
31:
32:  for (const booking of assignedBookings) {
33:    let slotStart: number;
34:    try {
35:      slotStart = slotStartUtcMs(booking.slotDate, booking.slotWindow);
36:    } catch (err: unknown) {
37:      Sentry.captureException(err);
38:      ctx.log(`detectNoShows: invalid slot data for ${booking.id} — skip`);
39:      continue;
40:    }
41:    if (now < slotStart + NO_SHOW_WINDOW_MS) continue;
42:
43:    // Credit write is the atomic idempotency gate — must come first.
44:    // If it throws (non-409 Cosmos error), skip this booking entirely.
45:    // If it returns false (409 = prior run already wrote credit), proceed with
46:    // downstream steps anyway — the prior run may have failed mid-way.
47:    let creditCreated: boolean;
48:    try {
49:      creditCreated = await customerCreditRepo.createCreditIfAbsent({
50:        id: booking.id,
51:        customerId: booking.customerId,
52:        bookingId: booking.id,
53:        amount: NO_SHOW_CREDIT_PAISE,
54:        reason: 'NO_SHOW',
55:        createdAt: new Date().toISOString(),
56:      });
57:    } catch (err: unknown) {
58:      Sentry.captureException(err);
59:      ctx.log(`detectNoShows: credit write failed for ${booking.id} — skip`);
60:      continue;
61:    }
62:
63:    if (creditCreated) {
64:      ctx.log(`detectNoShows: processing no-show bookingId=${booking.id}`);
65:    } else {
66:      ctx.log(`detectNoShows: credit already exists for ${booking.id} — retrying remaining steps`);
67:    }
68:
69:    // When recovering (creditCreated=false), re-fetch to check whether the status-write and
70:    // redispatch already completed on a prior run.
71:    // `noShowRedispatchAt` is stamped after a confirmed successful redispatch — if present,
72:    // the prior run completed the flow and nothing more is needed.
73:    // If absent but status is ASSIGNED with a *different* technicianId, a replacement tech
74:    // accepted the redispatch but the stamp write failed — treat as done and skip.
75:    // If absent and technicianId unchanged (original tech still on the booking), the prior run
76:    // crashed before the status write — retry.
77:    if (!creditCreated) {
78:      const liveBooking = await bookingRepo.getById(booking.id);
79:      if (liveBooking?.noShowRedispatchAt) {
80:        ctx.log(`detectNoShows: recovery skipped for ${booking.id} — redispatch already fired at ${liveBooking.noShowRedispatchAt}`);
81:        continue;
82:      }
83:      if (
84:        liveBooking?.status === 'ASSIGNED' &&
85:        liveBooking.technicianId !== undefined &&
86:        liveBooking.technicianId !== booking.technicianId
87:      ) {
88:        // A replacement tech accepted the redispatch; noShowRedispatchAt stamp failed but we're done
89:        ctx.log(`detectNoShows: recovery skipped for ${booking.id} — replacement tech ${liveBooking.technicianId} already assigned`);
90:        continue;
91:      }
92:    }
93:
94:    // Step 1: Set the holding status. Track success — if this fails we must NOT stamp
95:    // noShowRedispatchAt, so recovery can retry both the status write and redispatch.
96:    let statusWriteOk = false;
97:    try {
98:      await updateBookingFields(booking.id, { status: 'NO_SHOW_REDISPATCH' });
99:      statusWriteOk = true;
100:    } catch (err: unknown) {
101:      Sentry.captureException(err);
102:      ctx.log(`detectNoShows: status update failed ${booking.id}: ${err instanceof Error ? err.message : String(err)}`);
103:    }
104:
105:    // Step 2: Fire redispatch only when status write succeeded — prevents redispatch running
106:    // against a booking that is still ASSIGNED (dispatcher.redispatch checks for NO_SHOW_REDISPATCH).
107:    // Only stamp noShowRedispatchAt when redispatch actually sent offers (boolean true return)
108:    // so that recovery can distinguish "offers sent" from "no techs found / dispatcher skipped".
109:    let redispatchOk = false;
110:    if (statusWriteOk) {
111:      try {
112:        redispatchOk = await dispatcherService.redispatch(booking.id, NO_SHOW_REDISPATCH_RADIUS_KM);
113:        if (redispatchOk) {
114:          // Stamp the completed-redispatch flag after confirmed offers-sent.
115:          await updateBookingFields(booking.id, { noShowRedispatchAt: new Date().toISOString() });
116:        } else {
117:          ctx.log(`detectNoShows: no techs found for ${booking.id} — booking marked UNFULFILLED`);
118:        }
119:      } catch (err: unknown) {
120:        Sentry.captureException(err);
121:        ctx.log(`detectNoShows: redispatch failed ${booking.id}: ${err instanceof Error ? err.message : String(err)}`);
122:      }
123:    }
124:
125:    // FCM push: only send when redispatch actually started — the push text says "searching
126:    // for a new technician", which is false if redispatch threw or never ran.
127:    if (redispatchOk) {
128:      try {
129:        await sendNoShowCreditPush(booking.customerId, booking.id, NO_SHOW_CREDIT_PAISE);
130:      } catch (err: unknown) {
131:        Sentry.captureException(err);
132:        ctx.log(`detectNoShows: FCM failed ${booking.id}: ${err instanceof Error ? err.message : String(err)}`);
133:      }
134:    }
135:  }
136:}
137:
138:app.timer('triggerNoShowDetector', {
139:  schedule: '0 */5 * * * *',
140:  handler: async (_timer: Timer, ctx: InvocationContext): Promise<void> => {
141:    try {
142:      await detectNoShows(ctx);
143:    } catch (err: unknown) {
144:      Sentry.captureException(err);
145:      ctx.log(`detectNoShows ERROR: ${err instanceof Error ? err.message : String(err)}`);
146:      throw err;
147:    }
148:  },
149:});
...
codex
The new no-show flow can act on stale `ASSIGNED` snapshots, can return unsupported statuses to the existing technician active-job client, and suppresses the customer credit notification on redispatch failure paths. Those are functional regressions introduced by this patch.

Full review comments:

- [P1] Re-check current booking state before granting no-show credit — C:\Alok\Business Projects\Urbanclap-dup\api\src\functions\trigger-no-show-detector.ts:49-56
  If the technician marks the job `REACHED`/`IN_PROGRESS` after `getAssignedBookingsBefore()` returns but before this insert runs, we still write the ₹500 credit and kick off a re-dispatch based on a stale `ASSIGNED` snapshot. That creates a wrong credit and can disrupt a live booking. Re-reading the booking (or making the status change conditional) immediately before the credit/idempotency gate would avoid processing bookings that have already advanced.

- [P1] Clear or hide the old technician before no-show redispatch statuses — C:\Alok\Business Projects\Urbanclap-dup\api\src\functions\trigger-no-show-detector.ts:98-112
  This flow changes the booking status away from the active-job states but leaves `technicianId` intact. `GET /v1/technicians/active-job/{bookingId}` authorizes solely on that `technicianId`, so the no-show technician will keep polling and receive statuses like `NO_SHOW_REDISPATCH`, `SEARCHING`, or `UNFULFILLED`. In `technician-app`, `ActiveJobRepositoryImpl` parses the response with `ActiveJobStatus.valueOf(status)`, which throws for any status outside `ASSIGNED/EN_ROUTE/REACHED/IN_PROGRESS/COMPLETED`, so the active-job screen will break as soon as this timer touches the booking.

- [P2] Still send the compensation push when redispatch cannot send offers — C:\Alok\Business Projects\Urbanclap-dup\api\src\functions\trigger-no-show-detector.ts:125-127
  The customer push is now gated on `redispatchOk`, so any booking that gets credited but has no nearby replacement techs (or hits a dispatcher error) receives no `NO_SHOW_CREDIT_ISSUED` notification at all. That means the user is silently compensated with no in-app signal in exactly the failure cases this timer is supposed to handle gracefully.
The new no-show flow can act on stale `ASSIGNED` snapshots, can return unsupported statuses to the existing technician active-job client, and suppresses the customer credit notification on redispatch failure paths. Those are functional regressions introduced by this patch.
