...
| **E01-S07** | Fix admin-web Storybook's broken JSX runtime — `@storybook/react-vite` never configures the automatic/classic JSX runtime (no `@vitejs/plugin-react` wired into `.storybook/main.ts`), so **every** JSX-bearing story throws `ReferenceError: React is not defined`, including the pre-existing `LocaleSwitcher.stories.tsx` reference story. No CI workflow runs Storybook, so this has shipped silently. | NFR-M-5 (enterprise-floor: Storybook) | Discovered 2026-08-04 while fixing PR #305 (`CounterStrip.stories.tsx` `NextIntlClientProvider` decorator) — reproduced identically on the known-good `LocaleSwitcher` story with that fix absent, confirming it's pre-existing and repo-wide, not introduced by #305. Codemod-tier: add/configure `@vitejs/plugin-react` (or equivalent JSX-runtime config) in `admin-web/.storybook/main.ts`. Acceptance: `pnpm storybook build` (or equivalent non-interactive check) renders `LocaleSwitcher.stories.tsx` and `CounterStrip.stories.tsx` without `ReferenceError`; add a CI step so this can't silently regress again. |
