...
## Addendum 2026-09-10 — E23-S01 Weekly Incentive Milestone Gaming

**Author:** Alok Tiwari + Claude Sonnet 5 (architect persona)
**Trigger:** E23-S01 introduced a weekly milestone bonus (owner req R5): a technician who completes
enough countable jobs in an IST week earns a bonus, paid as credit against commission owed. Any
scheme that pays a technician for a *count* of jobs creates an incentive to inflate that count
cheaply — this addendum is the STRIDE treatment of that incentive. See
`docs/adr/0035-incentives-are-credit-only-and-capped.md` for the full design rationale and the
priced economics; this entry cross-references rather than re-derives those numbers.

Convention for `Status` column: `mitigated` | `partial` | `not-yet-mitigated` | `accepted`.

### New Threats: Incentive Milestone Gaming

| # | Threat | Code path | Likelihood | Impact | Mitigation | Residual risk | Status |
|---|---|---|---|---|---|---|---|
| **R-I1** | **Job-count padding with minimum-price phantom bookings.** A technician (alone, or colluding with a customer/confederate) creates or influences bookings priced as cheaply as possible purely to push the week's counted job total past a milestone threshold, without doing proportionally more real, revenue-generating work. | `api/src/services/incentive.service.ts` → `computeWeek`'s counting filter | M (2) — requires either a colluding customer or control over booking creation, both of which already exist as a possibility independent of this feature | M (3) — bonus paid for work that was largely fabricated, diluting the incentive programme's purpose without necessarily costing the platform cash directly (see R-I2 for the cash-extraction question) | **`minCountableBookingPaise`** (default ₹249, the cheapest real service at pilot pricing) excludes any "booking" priced below the cheapest real service from both the job count and the commission base — a booking that undercuts every real service price cannot pad the count at all. Admin-editable via `PUT /v1/admin/incentives/config`, takes effect on the next run, no deploy required. | Medium — this guard only excludes bookings priced *below* the real-service floor. A technician who pads with bookings priced *at or above* the floor (i.e. real, minimally-priced services, possibly with a colluding customer who cancels or no-shows little of consequence) is not caught by this guard at all — see R-I2, which is the guard that actually limits the payoff of that variant. | mitigated (for below-floor padding only) |
| **R-I2** | **The margin-fraction cap does not close the mixed real/cheap padding attack at its shipped default.** A technician does a minority of real, full-price jobs and pads the remainder of the week with jobs priced exactly at (not below) `minCountableBookingPaise`, reaching a milestone that a cheap-only attack could not reach as profitably. `docs/adr/0035-incentives-are-credit-only-and-capped.md` Consequence 1 prices the specific case Task 5's tests proved — 7 real ₹1,000 jobs + 3 padding jobs at the ₹249 floor nets the technician real profit at the shipped `capFractionBps = 6000` (60%) default, because the cap on the resulting counted commission sits well above the milestone's gross bonus and never constrains it. See the ADR for the exact figures rather than this entry, which cross-references rather than restates them to avoid the two documents drifting. | `api/src/services/incentive.service.ts` → `computeWeek`'s cap (`capPaise = floor(countedCommissionPaise × capFractionBps / 10_000)`) | M (2) — requires no special access, only a willingness to do a minority of real jobs and pad the rest at the pricing floor; more sophisticated than R-I1 but still well within an ordinary technician's reach | M (3) — the same profit-from-gaming outcome as R-I1, using pricing at rather than below the floor, so R-I1's mitigation alone does not stop it | **Documented, deliberate, admin-editable tradeoff, not a fix shipped at this default.** ADR-0035 Consequence 1 also prices the fix: `capFractionBps = 1500` (15%) closes this exact case with no false positive against an honest ten-job week (the cap only bites the padded week, not the genuine one). The lever is one config value (`PUT /v1/admin/incentives/config`), takes effect on the next run, no deploy. See `docs/runbook.md` → "Incentives (E23-S01)" → "Turning the cap down if gaming is observed" for the operator procedure. **Not lowered by default because it directly trades off against the milestone programme's stated purpose** (rewarding volume) — the owner has not observed gaming in production and has chosen to ship the more generous default until evidence says otherwise. | **High, stated plainly, not softened.** At the shipped default this guard does not close the priced attack above; a technician who understands the mechanics can extract real profit from padding without detection beyond whatever manual review of `GET /v1/admin/incentives/awards` an admin chooses to do (no automated anomaly detection exists on award composition). | accepted (shipped default) / mitigated (if `capFractionBps` is lowered per the runbook) |
| **R-I3** | **A gamed job count is extracted as cash, not merely as an accounting credit.** Even accepting that R-I1/R-I2 may inflate a job count and the resulting bonus, the platform's exposure is fundamentally different depending on whether that bonus can leave the platform as money the technician can spend freely versus a credit that only ever offsets commission the technician already owed. | `api/src/schemas/incentive.ts` → `IncentiveAwardWriteSchema`; `api/tests/static/incentives-credit-only.test.ts`; `api/.semgrep.yml` → `incentives-credit-only`, `incentives-not-in-payout-path` | L (1) — would require a future code change to introduce a payout field, not an attacker action against the current system | H (4) — if it were ever possible, a gamed count would convert directly into a cash payout event, not just a smaller-than-deserved commission bill | **Three independent layers**, per ADR-0035: (1) `IncentiveAwardWriteSchema` is `.strict()` at every level, so a `payoutPaise`/`netPayable`/`razorpayTransferId` field added in a future edit throws at parse time before it can reach Cosmos; (2) a static Vitest test proved (Task 17) to fail on three injected violation shapes (a value-position declaration, a reference from a payout file, and a schema-shape injection), each independently reverted after confirmation; (3) two Semgrep rules, round-2-fixed after a real-Semgrep run (Docker, since the native CLI is blocked in this environment) found round-1 missed type-position references and object-literal keys — re-verified against the clean tree (0 findings, including no false positive against the schema's own doc comment) and against all 3 re-injected violations (all caught). | Low for the mechanism this addendum is about — layer 1 alone is a structural, not a procedural, guarantee, and layers 2/3 catch a regression attempt before it ships. The one honestly-documented gap (a bare generic-wrapped type reference, e.g. `Promise<IncentiveAwardDoc>`, silently unmatched by either Semgrep rule) is a gap in the *detection* layer only — the `.strict()` schema still throws regardless of what Semgrep does or does not catch in source, so this residual does not translate into an actual path to a payout field reaching Cosmos. | mitigated |

**Addendum 2026-09-10 complete. New STRIDE entries: R-I1 (Repudiation/gaming, mitigated for
below-floor padding), R-I2 (Repudiation/gaming, accepted at the shipped default — see ADR-0035
before assuming this feature is gaming-proof), R-I3 (Elevation of privilege / cash extraction,
mitigated).**
